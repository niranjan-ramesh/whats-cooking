...
def extract_recipes_from_html(df):
    ...
    recipes_df = pd.DataFrame(
        columns=[
            'title', 'dish_url', 'rating', 'users_rated', 'owner', 'owner_url'
        ]
    )
    recipe_data = {'title': '', 'dish_url': '', 'rating': '',
                   'users_rated': '', 'owner': '', 'owner_url': ''}
    for html in df['html']:
        html_soup = BeautifulSoup(html, 'html.parser')
        # print(html_soup)
        recipes_container = html_soup.find_all(
            'div', class_='card__detailsContainer-left')
        for recipe_container in recipes_container:
            ...
            rating_container = recipe_container.find(
                'div', class_='card__ratingContainer')
            if(rating_container):
                # print(rating_container)
                rating = rating_container.find(
                    'span', class_='review-star-text visually-hidden').text
                ...
                recipe_data['users_rated'] = rating_container.find(
                    'span',
                    class_='ratings-count elementFont__details').text.strip()

            owner_container = recipe_container.find(
                'a', class_='card__authorNameLink elementFont__details--bold elementFont__detailsLinkOnly--underlined')
            if(owner_container):
