...
def item_to_item(recipesdf, modelval,page_select):
	...
	for i in range(len(modelval)):
		...
		rel = st.button(rec_name.title(),on_click=recipedetails,args = (recipesdf,rec_name,page_select))
...
def main():
	...
	if (ses == "True"):
		...
		if (page_select == 'Login'):
		    ...
		    next1 = st.button('Login',on_click=logincheck,args = (usersdf,username,page_type))
		...
		if (page_select == 'Signup'):
			...
			password = st.text_input("Password",type='password')
			next2 = st.button('Signup',on_click=signupcheck,args = (username,password,page_type))
