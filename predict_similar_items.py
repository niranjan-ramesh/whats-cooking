...
def text2vec(text):
    ...
    return np.mean(
        [model.get_vector(x, norm=True)
         for x in text.split() if x in vocab_keys],
        axis=0
    ).reshape(1, -1)


def get_column_tranformer():
    # Normalizing the ratings and polarity score columns
...
def get_similar_items(ingredients, items_to_predict=10):
    ...
    input_x = column_transformer.fit_transform(
        ratings_df[['rating', 'polarity_score']])
    ...
    if predicted_cluster.size != 0:
        predicted_recipes_cluster = features_df[features_df['clusters']
                                                == predicted_cluster[0]]
        predcited_df = predicted_recipes_cluster.sample(n=items_to_predict)
    else:
        print(
            f"unable to find similar items, so returning random\
            {items_to_predict} samples from dataset")
