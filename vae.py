...
class DeepAutoEncoder:
    ...
    def validation_loss(self, x_test):
        ...
        ab_ops=tf.math.divide(tf.reduce_sum(tf.abs(tf.subtract(x_test,recon_x))),num_test_labels)
    ...
    def fit(self, interactions, X_test, epochs, batchsize, lr=0.001):
        ...
        num_train_labels=tf.cast(tf.math.count_nonzero(mask),dtype=tf.float64) 
        bool_mask=tf.cast(mask,dtype=tf.bool)
    ...
    def write_recc_files(self):
        ...
        cwd = os.getcwd()
        reccomendations.to_csv(cwd + '/Data/reccomendations_vae.csv', index=False)    
